...
internal static class MarkdownReport
{
    ...
    public static async Task Write(DirectoryInfo license_root)
    {
        ...
        await WriteSummary(sw, sorted);
        await WriteTable(sw, sorted, license_root);
        await WriteSections(sw, sorted, license_root);
        await WriteFooter(sw);
    }

    static async Task WriteSummary(StreamWriter sw, LicenseEntry[] entries)
    {
        var total = entries.Length;
        var uniqueLicenses = entries.Select(e => e.License).Distinct().Count();
        var unresolved = entries.Count(e => e.License == NulicLicense.NOASSERTION);

        var summary = $"{total} packages · {uniqueLicenses} unique licenses";
        if (unresolved > 0)
            summary += $" · ⚠️ {unresolved} unresolved";

        await sw.WriteLineAsync($"_{summary}_");
    ...
    static async Task WriteTable(StreamWriter sw, LicenseEntry[] entries, DirectoryInfo license_root)
    {
        ...
        foreach (var e in entries)
        {
            ...
            var warning = e.License == NulicLicense.NOASSERTION ? "⚠️ " : "";

            await sw.WriteLineAsync($"| {warning}{name} | {e.Version} | {license} | {Escape(authors)} |");
        }
    ...
    static async Task WriteSections(StreamWriter sw, LicenseEntry[] entries, DirectoryInfo license_root)
    {
        ...
        foreach (var group in groups)
        {
            ...
            var countLabel = count == 1 ? "1 package" : $"{count} packages";
            await sw.WriteLineAsync($"## {group.Key} ({countLabel})");
            await sw.WriteLineAsync();
    ...
    static async Task WriteFooter(StreamWriter sw)
    {
        var version = typeof(MarkdownReport).Assembly
            .GetCustomAttribute<AssemblyInformationalVersionAttribute>()
            ?.InformationalVersion ?? "unknown";
        // strip build metadata (+commit hash) from MinVer version
        var shortVersion = version.Split('+')[0];
        var timestamp = DateTimeOffset.Now.ToString("yyyy-MM-dd HH:mm zzz");
        await sw.WriteLineAsync($"<sub>Generated by [nulic](https://github.com/rafntor/nulic) {shortVersion} on {timestamp}</sub>");
    }
